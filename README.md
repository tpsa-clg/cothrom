...
- use class inheritance for different map types e.g. single-seat ED groupings (original proof-of-concept), multiple-seat ED groupings (Dublin/Cork, full-scale Ireland, LEAs), multiple-seat county groupings (EU Parliament constituencies)
- figure out best way to sort files into directories, e.g. datasets, header files, source code, results/plots, etc.
