...
full_df = pd.read_csv(os.path.join(data_dir, "ED_data.csv"), usecols=["GUID", area_type, "Population", "Neighbours", "County", "Area", "Perimeter"])
...
files = [open(os.path.join(file_dir, f"{column}.txt"), "w") for column in ["GUID", "Population", "Neighbours", "County"]]
